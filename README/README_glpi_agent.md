...
## Почему скачивание идёт через control-ноду, а не напрямую с хоста

Целевые хосты (test1/test2) в этой лабе изолированы и/или испытывают проблемы с DNS-резолвингом наружу — прямое скачивание с них ненадёжно. Файл скачивается один раз на control-ноду (где сеть заведомо рабочая) и оттуда раздаётся хостам через обычный механизм передачи файлов Ansible — не требует прямого доступа хоста в интернет вообще.

Если по какой-то причине сеть барахлит уже и на control-ноде — файл инсталлятора можно положить в `files/glpi-agent-installer/` вручную (скачав через браузер на любой другой машине); роль автоматически определит его наличие и не будет пытаться скачивать заново.
